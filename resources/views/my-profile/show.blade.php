<x-default-layout>
    ...
    <div class="max-w-2xl mx-auto bg-white border-4 border-black p-4 md:p-8 text-center">
        <header class="mb-8">
            <div class="inline-block relative mb-6">
                <div
                    class="w-32 h-32 md:w-40 md:h-40 rounded-full overflow-hidden border-4 border-black bg-yellow-300 flex items-center justify-center">
                    @if ($user->profile_picture)
                        <img src="{{ asset('storage/' . $user->profile_picture) }}" alt="{{ $user->username }}"
                            class="w-full h-full object-cover">
                    @else
                        <span class="text-5xl md:text-6xl font-black">{{ strtoupper(substr($user->first_name, 0, 1) . substr($user->last_name, 0, 1)) }}</span>
                    @endif
                </div>
            </div>
            ...
            <p class="text-xl md:text-2xl text-black/70 font-bold">
                {{ '@' . $user->username }}
            </p>
        </header>
        ...
        <footer class="flex flex-col md:flex-row flex-wrap justify-center gap-4">
